...
all: clean
	...
	apprepo --destination=$(PWD)/build appdir boilerplate shotwell shotwell-common
	echo "exec \$${APPDIR}/bin/shotwell \"\$${@}\"" >> $(PWD)/build/Boilerplate.AppDir/AppRun
	...
	export ARCH=x86_64 && bin/appimagetool.AppImage $(PWD)/build/Boilerplate.AppDir $(PWD)/Shotwell.AppImage
	chmod +x $(PWD)/Shotwell.AppImage
